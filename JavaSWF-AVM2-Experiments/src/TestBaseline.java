...
public class TestBaseline {
	...
	public static void main( String[] args ) {
		...
		SWFWriter swf  = new SWFWriter( filename );
		TagWriter tags = new TagWriter( swf );
		
		tags.header( 9, -1, 10000, 10000, 12, -1 );
        tags.tagFileAttributes( SWFConstants.FILE_ATTRIBUTES_ALLOW_AS3
                                  + SWFConstants.FILE_ATTRIBUTES_USE_NETWORK );

        tags.tagScriptLimits( 1000, 60 );

        Color color = new Color( 0xffffff );
        tags.tagSetBackgroundColor( color );
		
        ABC abc = tags.tagDoABC( SWFConstants.DO_ABC_LAZY_INITIALIZE_FLAG, "test" );        
        makeFile().write( abc );
        
		tags.tagShowFrame();
		tags.tagSymbolClass( Collections.singletonMap( 0, "test.Test" ) );		
		tags.tagEnd();
	}

	private static AVM2ABCFile makeFile() {
		AVM2ABCFile abc = new AVM2ABCFile();
		
		AVM2QName qnameTest = new AVM2QName( "test.Test" );
		AVM2QName qnameMC   = new AVM2QName( "flash.display.MovieClip" );
		AVM2Namespace protNS = new AVM2Namespace( NamespaceKind.ProtectedNamespace, "test:Test" );		
		
        AVM2Class avm2Class = abc.addClass( qnameTest, qnameMC, true, false, false, protNS );
